...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "work_day_target", "(day_of_week, hours)", (0, -1.0),
            id="work_day_target-hours>=0",
        ),
        pytest.param(
            "work_day_exception", "(date, hours)", ("2026-01-01", -0.5),
            id="work_day_exception-hours>=0",
        ),
        pytest.param(
            "vacation_record", "(date, hours, vtype)",
...
def test_hours_check_rejects_negative_on_hours_gte_zero_tables(
        db: Database, table: str, columns: str, values: tuple) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "sickness_record", "(date, hours)", ("2026-01-01", 0.0),
            id="sickness_record-hours>0-zero-rejected",
        ),
        pytest.param(
            "carry_over_log", "(from_year, to_year, hours)", (2025, 2026, 0.0),
...
def test_vacation_record_hours_zero_is_accepted_after_v2_relaxation(db: Database) -> None:
...
def test_miliuim_period_check_accepts_end_date_equal_to_start_date(db: Database) -> None:
...
def test_work_day_target_check_rejects_out_of_range_day_of_week(
        db: Database, day_of_week: int) -> None:
    conn = db.get_connection()
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
