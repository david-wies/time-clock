...
class SicknessTab(ttk.Frame):
    ...
    def _refresh_balance(self) -> None:
        ...
        self._lbl_balance.config(
            text=(
                f"Sick days {year}: {used_days:.1f} / {allowance:.1f} used"
                f"  |  Remaining: {remaining:.1f} days"
            ),
            foreground=bal_color,
        )
        self._lbl_hours.config(text=f"Total hours: {used_hours:.1f}h")
    ...
    def _refresh_tree(self) -> None:
        ...
        for rec in records:
            ...
            total_days += self.model.get_day_equivalent(rec.date, rec.hours)
        ...
        if records:
            self._tree.insert(
                ...
                values=self._make_row_values(
                    None,
                    f"Total: {total_hours:.1f}h ({total_days:.2f}d)",
                ),
                tags=("total",),
            )
            c = COLORS.get("light", COLORS["light"])
